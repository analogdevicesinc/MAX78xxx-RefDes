...
void lctrInitActInitiate(void)
{
  bool_t result = TRUE;
  lctrConnCtx_t *pCtx;
  uint32_t connInterUsec, interMinUsec, interMaxUsec, durUsec;
  ...
  do
  {
    ...
    interMinUsec = LCTR_CONN_IND_US(pInitMsg->connSpec.connIntervalMin);
    interMaxUsec = LCTR_CONN_IND_US(pInitMsg->connSpec.connIntervalMax);
    durUsec = pCtx->localConnDurUsec;
...
void lctrInitActScanTerm(void)
{
  ...
  evt.hdr.event  = LL_CREATE_CONN_CANCEL_CNF;
...
void lctrInitActDisallowCancel(void)
{
  LlCreateConnCancelCnf_t evt;
  evt.hdr.event  = LL_CREATE_CONN_CANCEL_CNF;
  evt.hdr.status = LL_ERROR_CODE_CMD_DISALLOWED;
  evt.status = LL_ERROR_CODE_CMD_DISALLOWED;
