...
void SchBleCalcAdvOpDuration(BbOpDesc_t *pBod)
{
  ...
  BbBleMstAdvEvent_t *pAdvMst;
  BbBleSlvAdvEvent_t *pAdvSlv;
  ...
  switch (pBle->chan.opType)
  {
    ...
    case BB_BLE_OP_SLV_ADV_EVENT:
    {
      ...
      pAdvSlv = &pBle->op.slvAdv;

      numChan  = (pAdvSlv->advChMap & (1 << 0)) ? 1 : 0;
      numChan += (pAdvSlv->advChMap & (1 << 1)) ? 1 : 0;
      numChan += (pAdvSlv->advChMap & (1 << 2)) ? 1 : 0;
