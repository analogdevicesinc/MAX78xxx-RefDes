...
void GyroMeasComplete(dmConnId_t connId, int16_t x, int16_t y, int16_t z)
{
  ...
  gyroData[0] = UINT16_TO_BYTES(x);
  gyroData[1] = UINT16_TO_BYTES(y);
  gyroData[2] = UINT16_TO_BYTES(z);

  AttsSetAttr(GYRO_HANDLE_DATA, sizeof(gyroData), (uint8_t*)gyroData);
  AttsHandleValueNtf(connId, GYRO_HANDLE_DATA, sizeof(gyroData), (uint8_t*)gyroData);
