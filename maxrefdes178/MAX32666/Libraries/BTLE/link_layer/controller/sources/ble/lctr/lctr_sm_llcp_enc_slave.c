...
static void lctrActPeerStartEncRsp(lctrConnCtx_t *pCtx)
{
  ...
  // Increment the packet counter for the LL_START_ENC_RSP message from the master
  // Counter was not incremented when this packet was received because encryption was
  // Not yet enabled, but decryption was enabled.
  pCtx->rxPktCounter++;
