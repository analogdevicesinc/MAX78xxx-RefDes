...
bool_t lctrSlvRxAuxConnReqHandler(BbOpDesc_t *pOp, const uint8_t *pReqBuf)
{
      ...
      (lctrAdvIsrAdvHdr.len == LL_CONN_IND_PDU_LEN))
  {
    ...
    pAuxConnRspTgtA = pAdvSet->auxRspHdrBuf + LL_ADV_HDR_LEN + LCTR_EXT_HDR_CMN_LEN + LCTR_EXT_HDR_FLAG_LEN + BDA_ADDR_LEN;
    ...
    pAuxConnRspAdvA = pAdvSet->auxRspHdrBuf + LL_ADV_HDR_LEN + LCTR_EXT_HDR_CMN_LEN + LCTR_EXT_HDR_FLAG_LEN;
...
void lctrSlvExtAdvEndOp(BbOpDesc_t *pOp)
{
  ...
  if ((pAdvSet->param.advEventProp & LL_ADV_EVT_PROP_LEGACY_ADV_BIT) == 0)
  {
    ...
    if ((pAdvSet->advData.alt.ext.modified || pAdvSet->pExtAdvAuxPtr) &&
        !pAdvSet->auxBodUsed)
    {
  ...
  {
    {
      ...
      {
        uint32_t advEventDur;
        result = SchInsertEarlyAsPossible(pOp, pAdvSet->param.priAdvInterMin, pAdvSet->param.priAdvInterMax);
        if (!result)
        {
          pOp->due += pAdvSet->param.priAdvInterMax;
        }

        advEventDur = pOp->due - advEventStart;
...
void lctrSlvAuxAdvEndOp(BbOpDesc_t *pOp)
{
  ...
  if (pAdvSet->advData.alt.ext.modified)
  {
    ...
    /* Update superior PDU. */
    pAdv = &pAdvSet->bleData.op.slvAdv;
    pAdv->txAdvLen = lctrPackAdvExtIndPdu(pAdvSet, pAdvSet->advHdrBuf, FALSE);
