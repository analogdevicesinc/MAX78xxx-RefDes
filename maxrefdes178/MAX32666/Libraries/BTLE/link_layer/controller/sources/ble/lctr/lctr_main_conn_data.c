 ...
 */
/*************************************************************************************************/
static uint16_t lctrGetMaxConnDurationUsec(uint8_t phy, uint16_t maxLen, uint16_t maxTime)
{
  uint16_t maxDur;
...
void lctrTxDataPduQueue(lctrConnCtx_t *pCtx, uint16_t fragLen, lctrAclHdr_t *pAclHdr, uint8_t *pAclBuf)
{
  ...
  pData = &pDesc->data[LL_DATA_HDR_LEN];
  pAclBufStart = pAclBuf;
  pDesc->aclLen = 0;
...
#if (LL_ENABLE_TESTER)
  opcode = pBuf[LCTR_DATA_TX_PDU_START_OFFSET + LL_DATA_HDR_LEN];
