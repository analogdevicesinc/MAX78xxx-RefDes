...
{
  uint8_t *pData;
  uint8_t dataLen, peerIdAddrType;
  uint64_t peerIdAddr;
  ...
  dataLen = advHdr.len - LL_ADV_PREFIX_LEN;
...
void lctrMstRxDirectAdvBPduHandler(void)
{
  ...
  while ((pAdvBuf = WsfMsgDeq(&lctrMstScan.rxDirectAdvbQ, &handlerId)) != NULL)
  {
    ...
    if (lctrMstCreateAdvRpt(pAdvBuf, &rpt, &localMatch))
    {
      ...
      pInitA = pAdvBuf + LL_ADV_HDR_LEN + BDA_ADDR_LEN;
...
{
  LlConnInd_t evt;
  evt.hdr.event  = LL_CONN_IND;
  evt.hdr.status = reason;
  evt.status = reason;
  evt.addrType = peerAddrType;
