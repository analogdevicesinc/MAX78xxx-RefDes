...
{
  lctrExtScanCtx_t *pCtx;
  BbBleData_t *pBle;
  BbBleMstAdvEvent_t *pScan;
  uint32_t advEndTs, txWinOffsetUsec;
  uint8_t *pConnIndInitA;
  ...
  else if (pScan->pTxReqBuf)
  {
    ...
    if (pCtx->data.init.ownAddrType & LL_ADDR_IDENTITY_BIT)
    {
      ...
      pConnIndInitA = pScan->pTxReqBuf + LL_ADV_HDR_LEN;
    ...
    txWinOffsetUsec = BB_TICKS_TO_US(pCtx->data.init.firstCeDue - advEndTs) - LL_BLE_TIFS_US -
    ...
    txWinOffset = LCTR_US_TO_CONN_IND(txWinOffsetUsec) - LCTR_DATA_CHAN_DLY;
    ...
    case LL_PDU_ADV_EXT_IND:
    {
      ...
      if (extAdvHdrFlags & LL_EXT_HDR_AUX_PTR_BIT)
      {
        ...
        do
        {
          ...
          pInitMsg = (lctrInitiateMsg_t *)pLctrMsg;
...
bool_t lctrMstInitiateRxAuxAdvPktHandler(BbOpDesc_t *pOp, const uint8_t *pAdvBuf)
{
  ...
  pExtAdvHdr = pAdvBuf + lctrUnpackAdvbPduHdr(&advHdr, pAdvBuf);
  lctrUnpackExtAdvHeader(&pExtInitCtx->extAdvHdr, &extAdvHdrFlags, pExtAdvHdr);
  ...
  else if (pAuxScan->pTxAuxReqBuf)
  {
    ...
    if (pExtInitCtx->data.init.ownAddrType & LL_ADDR_IDENTITY_BIT)
    {
      ...
      pAuxConnReqInitA = pAuxScan->pTxAuxReqBuf + LL_ADV_HDR_LEN;
    ...
    txWinOffsetUsec = BB_TICKS_TO_US(pExtInitCtx->data.init.firstCeDue - connReqEndTs);
    txWinOffset = LCTR_US_TO_CONN_IND(txWinOffsetUsec);
...
bool_t lctrMstInitiateRxAuxConnRspHandler(BbOpDesc_t *pOp, const uint8_t *pRspBuf)
{
  ...
  pExtAdvHdr = pRspBuf + lctrUnpackAdvbPduHdr(&advHdr, pRspBuf);
  lctrUnpackExtAdvHeader(&pExtInitCtx->extAdvHdr, &extAdvHdrFlags, pExtAdvHdr);
