...
void LmgrSendExtScanEnableCnf(uint8_t status)
{
  ...
  evt.hdr.event  = LL_EXT_SCAN_ENABLE_CNF;
  evt.hdr.status = lmgrCb.extScanEnaStatus;
  evt.status = lmgrCb.extScanEnaStatus;
...
void LmgrSendSyncEstInd(uint8_t status, uint16_t handle, lmgrPerAdvSyncEstdInd_t *pEvt)
{
  ...
  evt.hdr.event  = LL_PER_ADV_SYNC_ESTD_IND,
  evt.status = status;
  evt.syncHandle = handle;
  evt.advSID = pEvt->advSID;
  evt.addrType = pEvt->addrType;
  evt.advPhy = pEvt->advPhy;
  evt.perAdvInterval = pEvt->advInterval;
  evt.advClkAccuracy = pEvt->advClkAccuracy;
...
void LmgrSendSyncLostInd(uint16_t handle)
{
  ...
  evt.hdr.event  = LL_PER_ADV_SYNC_LOST_IND,
  evt.syncHandle = handle;
