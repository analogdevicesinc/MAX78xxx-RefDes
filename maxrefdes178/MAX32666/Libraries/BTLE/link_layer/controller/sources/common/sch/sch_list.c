 ...
 */
/*************************************************************************************************/
void SchCheckIsNotInserted(BbOpDesc_t *pBod)
{
 ...
 */
/*************************************************************************************************/
void SchCheckIsInserted(BbOpDesc_t *pBod)
{
  BbOpDesc_t *pCur = schCb.pHead;
 ...
 */
/*************************************************************************************************/
bool_t SchEnoughTimeToCancel(BbOpDesc_t *pBod)
{
  bool_t result = FALSE;
 ...
 */
/*************************************************************************************************/
uint32_t SchGetStartRefTime(void)
{
  if (schCb.pHead)
  {
 ...
 */
/*************************************************************************************************/
void schInsertToEmptyList(BbOpDesc_t *pItem)
{
  WSF_ASSERT(pItem);
 ...
 */
/*************************************************************************************************/
void schInsertBefore(BbOpDesc_t *pItem, BbOpDesc_t *pTgt)
{
 ...
 */
/*************************************************************************************************/
void schInsertAfter(BbOpDesc_t *pItem, BbOpDesc_t *pTgt)
{
  WSF_ASSERT(pTgt && pItem);
...
void SchInsertNextAvailable(BbOpDesc_t *pBod)
{
  ...
  startRef = SchGetStartRefTime();

  /* Allow for rescheduling if necessary */
  pBod->due = BbDrvGetCurrentTime() + BB_US_TO_BB_TICKS(2*BbGetSchSetupDelayUs());
...
{
  uint32_t startRef, dueOrigin, nextAvailInter;
  bool_t result = FALSE;
  ...
  if (schDueTimeInFuture(pBod))
  {
    ...
    else
    {
      ...
      while (pCur)
      {
        ...
        nextAvailInter = pBod->due - dueOrigin;
