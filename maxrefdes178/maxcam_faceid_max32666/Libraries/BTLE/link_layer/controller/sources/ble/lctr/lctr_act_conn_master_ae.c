...
static uint8_t lctrExtInitSetupConn(lctrExtScanCtx_t *pExtInitCtx, LlConnSpec_t *pConnSpec)
{
  ...
  uint32_t interMinUsec, interMaxUsec, durUsec;
  ...
  interMinUsec = LCTR_CONN_IND_US(pConnSpec->connIntervalMin);
  interMaxUsec = LCTR_CONN_IND_US(pConnSpec->connIntervalMax);
...
void lctrMstExtInitCleanupOp(lctrExtScanCtx_t *pExtInitCtx)
{
  ...
  pScan = &pExtInitCtx->scanBleData.op.mstAdv;
  if (pScan->pRxAdvBuf)
  {
