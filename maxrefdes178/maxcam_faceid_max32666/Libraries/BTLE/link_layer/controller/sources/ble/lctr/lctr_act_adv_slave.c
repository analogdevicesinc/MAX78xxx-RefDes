...
static void lctrNotifyHostDirectConnectTimeout(void)
{
  ...
  evt.hdr.status     = LL_ERROR_CODE_ADV_TIMEOUT;
  evt.status         = LL_ERROR_CODE_ADV_TIMEOUT;
  evt.handle         = 0;
  evt.role           = LL_ROLE_SLAVE;
  evt.addrType       = lmgrSlvAdvCb.advParam.peerAddrType;
  evt.connInterval   = 0;
  evt.connLatency    = 0;
  evt.supTimeout     = 0;
  evt.clockAccuracy  = 0;

  pBuf = (uint8_t *)&evt.peerAddr;
