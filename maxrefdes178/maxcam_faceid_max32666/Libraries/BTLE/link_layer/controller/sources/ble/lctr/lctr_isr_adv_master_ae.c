 ...
 */
/*************************************************************************************************/
static void lctrGetLocalIdAddr(lctrExtScanCtx_t *pExtScanCtx, uint64_t targetAddr,
                                ...
                                int8_t rssi, const uint8_t *pAdvBuf, LlExtAdvReportInd_t *pRpt)
{
  ...
  switch (pAdvHdr->pduType)
  {
    ...
    default:
    {
      ...
      pData = pAdvBuf + LL_ADV_HDR_LEN + LL_ADV_PREFIX_LEN;
      dataLen = pAdvHdr->len - LL_ADV_PREFIX_LEN;
...
{
  LlExtAdvReportInd_t *pRpt;
  uint16_t availLen, fragLen;

  /* Ensure report event is already packed. */
  WSF_ASSERT(pExtScanCtx->data.scan.advRptState == LCTR_RPT_STATE_IN_PROGRESS);

  pRpt = &pExtScanCtx->data.scan.advRpt;

  availLen = pLctrRtCfg->maxExtScanDataLen - pRpt->len;
  fragLen = WSF_MIN(availLen, chainLen);
 ...
 */
/*************************************************************************************************/
static bool_t lctrPerAdvRptPack(BbOpDesc_t *pOp, const uint8_t *pAdvBuf,
 ...
 */
/*************************************************************************************************/
static bool_t lctrPerAdvRptPackTruncate(BbOpDesc_t *pOp, const uint8_t *pAdvBuf, LlPerAdvReportInd_t *pRpt)
{
  lctrPerScanCtx_t * const pPerScanCtx = pOp->pCtx;
...
void lctrMstDiscoverRxExtAdvPktPostProcessHandler(BbOpDesc_t *pOp, const uint8_t *pAdvBuf)
{
  ...
  switch (advHdr.pduType)
  {
    ...
    case LL_PDU_ADV_EXT_IND:
    {
      ...
      pExtAdvHdr = pAdvBuf + LL_ADV_HDR_LEN;
...
{
  BbBleData_t *pBle;
  BbBleMstAuxAdvEvent_t *pAuxScan;
  lctrExtScanCtx_t * pExtScanCtx;
  ...
  pBle = pOp->prot.pBle;
  pAuxScan = &pBle->op.mstAuxAdv;
  pExtScanCtx = pOp->pCtx;

  pExtAdvHdr = pAdvBuf + lctrUnpackAdvbPduHdr(&lctrMstExtScanIsr.advHdr, pAdvBuf);
    ...
    if (pAuxScan->pTxAuxReqBuf)     /* Active scanner. */
    {
      ...
      if ((txScanReq = lctrScanBackoffCheckReqAllowed(pExtScanCtx)) == TRUE)
      {
        ...
        if (pExtScanCtx->scanParam.ownAddrType & LL_ADDR_IDENTITY_BIT)
        {
          ...
          pScanReqScanA = pExtScanCtx->reqBuf + LL_ADV_HDR_LEN;
...
bool_t lctrMstDiscoverRxAuxScanRspHandler(BbOpDesc_t *pOp, const uint8_t *pRspBuf)
{
  ...
  pExtScanCtx = pOp->pCtx;
  pAuxScan = &pOp->prot.pBle->op.mstAuxAdv;
  ...
  pExtAdvHdr = pRspBuf + lctrUnpackAdvbPduHdr(&lctrMstExtScanIsr.advHdr, pRspBuf);
  lctrMstExtScanIsr.extAdvHdrLen = lctrUnpackExtAdvHeader(&pExtScanCtx->extAdvHdr, &lctrMstExtScanIsr.extAdvHdrFlags, pExtAdvHdr);
  lctrMstExtScanIsr.isHdrParsed = TRUE;
  ...
  if (pScan->pTxReqBuf)
  {
    switch (pScan->filtResults.pduType)
    {
      ...
      case LL_PDU_ADV_SCAN_IND:
      {
        ...
        /* Update scan request header with advertiser's address. */
        pScanReqAdvA = pExtScanCtx->reqBuf + LL_ADV_HDR_LEN + BDA_ADDR_LEN;
...
{
  uint16_t skip, numUnsyncIntervals;
  lctrPerScanCtx_t * const pPerScanCtx = pOp->pCtx;
  ...
  if (pPerScanCtx->firstPerAdvRcv == FALSE)
  {
    ...
    if ((pPerScanCtx->eventCounter - pPerScanCtx->initEventCounter) == (fastTermCnt - 1))
    {
      ...
      lctrSendCreateSyncMsg(LCTR_CREATE_SYNC_MSG_FAILED);
    }
  }
  ...
  numUnsyncIntervals     = pPerScanCtx->eventCounter - pPerScanCtx->lastActiveEvent;

  while (TRUE)
  {
    uint32_t unsyncTimeUsec, caPpm, wwTotalUsec, wwTotal, connInterUsec, connInter;
    ...
    caPpm          = lctrCalcTotalAccuracy(pPerScanCtx->sca);
    wwTotalUsec    = lctrCalcAuxAdvWindowWideningUsec(unsyncTimeUsec, caPpm);
    wwTotal        = BB_US_TO_BB_TICKS(wwTotalUsec);
    connInterUsec  = BB_TICKS_TO_US(numUnsyncIntervals * pPerScanCtx->perInter);
...
uint32_t lctrMstPerScanRxPerAdvPktHandler(BbOpDesc_t *pOp, const uint8_t *pAdvBuf, uint8_t status)
{
  ...
  const uint8_t *pExtAdvHdr;
  uint32_t auxOffsetUsec = 0;
  BbBleMstPerScanEvent_t *pMstPerScan;

  WSF_ASSERT(pOp->protId == BB_PROT_BLE);
  WSF_ASSERT(pOp->prot.pBle->chan.opType == BB_BLE_OP_MST_PER_SCAN_EVENT);

  pBle = pOp->prot.pBle;
  pPerScanCtx = pOp->pCtx;
  ...
  advHdrLen = pAdvBuf[1];
  pExtAdvHdr = pAdvBuf + LL_ADV_HDR_LEN;
  pMstPerScan = &pBle->op.mstPerScan;
