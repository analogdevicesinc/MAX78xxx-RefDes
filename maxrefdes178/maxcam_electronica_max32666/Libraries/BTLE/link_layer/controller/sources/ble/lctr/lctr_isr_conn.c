 ...
 */
/*************************************************************************************************/
static void lctrBuildEmptyPdu(lctrConnCtx_t *pCtx)
{
 ...
 */
/*************************************************************************************************/
static void lctrUpdateFlowCtrlBits(const lctrDataPduHdr_t *pHdr, uint8_t *pBuf)
{
 ...
 */
/*************************************************************************************************/
static uint8_t lctrRecoverLlid(uint8_t *pBuf)
{
...
uint8_t *lctrProcessRxAck(lctrConnCtx_t *pCtx)
{
  ...
  if (((pCtx->rxHdr.sn ^ pCtx->txHdr.nesn) & 1) == 0)           /* bits are same */
  {
    ...
    else if (pCtx->rxHdr.len)               /* zero length implies Empty PDU */
    {
      ...
      // Only increment packet counter for non-empty packets
...
bool_t lctrProcessTxAck(lctrConnCtx_t *pCtx)
{
  if (((pCtx->rxHdr.nesn ^ pCtx->txHdr.sn) & 1) == 1)           /* bits are different */
  {
    ...
    else
    {
      ...
      // Do not increment packet counter for empty PDUs
      ...
      reqTx)
  {
    ...
    else
    {
      ...
      if (sendAck)
      {
        ...
        desc.pBuf = lctrConnIsr.emptyPdu;
        desc.len = sizeof(lctrConnIsr.emptyPdu);
...
{
  uint32_t curTime, setupDelayUsec, availCeUsec;
  ...
  curTime = BbDrvGetCurrentTime();
  setupDelayUsec = BbGetSchSetupDelayUs();

  availCeUsec = LCTR_CONN_IND_US(pCtx->connInterval);
