...
static void lctrMstConnUpdateOp(lctrConnCtx_t *pCtx)
{
  ...
  txWinOffset = BB_BLE_TO_BB_TICKS(LCTR_CONN_IND_TICKS(pConnUpdInd->txWinOffset));
...
void lctrMstConnEndOp(BbOpDesc_t *pOp)
{
  ...
  uint16_t anchorPointOffsetUsec, numIntervals;
  ...
  if (pCtx->data.mst.sendConnUpdInd)
  {
    ...
    if ((pPdu = lctrTxCtrlPduAlloc(LL_CONN_UPD_IND_PDU_LEN)) != NULL)
    {
      ...
      txWinOffsetUsec = SchRmGetOffsetUsec(rsvnOffs, 0, LCTR_GET_CONN_HANDLE(pCtx));
  ...
  anchorPoint           = pOp->due;
  anchorPointOffsetUsec = pOp->dueOffsetUsec;
  numIntervals          = 0;
  ...
  while (TRUE)
  {
    uint32_t connInterUsec, connInter;
