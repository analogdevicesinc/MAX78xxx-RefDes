...
void lctrCalcSessionKey(lctrConnCtx_t *pCtx)
{
  ...
  dir = (pCtx->role == LL_ROLE_MASTER) ? 1 : 0;     /* master = 1; slave = 0 */
  pEnc->dir = dir;
...
void lctrEncNotifyHostLtkReqInd(lctrConnCtx_t *pCtx)
{
  ...
  evt.hdr.event        = LL_LTK_REQ_IND;
  evt.hdr.status       = LL_SUCCESS;

  evt.handle         = handle;
  evt.encDiversifier = pCtx->ediv;
...
void lctrNotifyEncChangeInd(lctrConnCtx_t *pCtx, uint8_t status)
{
  ...
  evt.hdr.param        = handle;
  evt.hdr.event        = LL_ENC_CHANGE_IND;
  evt.hdr.status       = status;
  evt.status         = status;
  evt.handle         = handle;
  evt.enabled        = pEnc->enaDecrypt;
...
void lctrNotifyEncKeyRefreshInd(lctrConnCtx_t *pCtx)
{
  ...
  evt.hdr.param        = handle;
  evt.hdr.event        = LL_ENC_KEY_REFRESH_IND;
  evt.hdr.status       = LL_SUCCESS;
...
void lctrNotifyAuthPayloadTimeout(lctrConnCtx_t *pCtx)
{
  ...
  evt.hdr.param        = handle;
  evt.hdr.event        = LL_AUTH_PAYLOAD_TIMEOUT_IND;
  evt.hdr.status       = LL_SUCCESS;
