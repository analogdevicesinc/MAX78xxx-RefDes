...
On front face of the cube, there are opening for the camera image sensor, MEMS microphone and two white LEDs for camera flash.
...
On the right side of the cube, there are two openings for USB Type-C connector, Micro SD card slot. The power button with RGB LED is also placed on this side.
...
On the left side of the cube, there are audio connectors for both input and output. The headset (MAX78000) connectors allows connecting headsets equipped with microphones. Both headset connectors are 2.5mm connector using OMTP (Open Mobile Terminal Platform) standard. The headset connector and Line-In connector on the front cover are connected to MAX9867 audio CODEC which is connected to MAX78000 Audio processor. The headset connector near to the AI logo is operating through the MAX9867 audio CODEC which is connected to MAX32666 microcontroller.

For all audio connectors, it is required to use a 2.5mm to 3.5mm adapter is needed in order to use a 3.5mm jack.
...
There is circular general purpose mounting hole at the bottom side of the cube. The diameter of the hole is 5.40mm. There are no screw threads on this hole.

There is also a rectangular window at this side to assemble the enclosure and boards. This window is normally covered with a sticker at the factory.
